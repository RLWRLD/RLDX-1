...
def _apply_cli_model_overrides(
    ...
    # Issue #54 — action model LoRA. Mirror every CLI knob onto the model
    # config so the saved config.json records the values the user actually
    # trained with (skipping any of these turns the CLI flag into a silent
    # no-op, the same regression family Issue #22 tracked for RTC fields).
    ...
    # Issue #55 — backbone (Qwen3 LLM) LoRA. Same silent-no-op concern as the
    # action-model knobs above: copy every CLI field onto run_config.model
    ...
    # Issue #61 — when backbone_use_lora is on, the LoRA adapters own the LLM
    # trainable surface; copy 0 onto the model config so the warning above
    # actually matches behaviour. Otherwise BackboneAdapter would mark the
    # top-N base weights requires_grad=True and adapter.py's bf16→fp32 pre-
    # cast would run, only for ``_apply_backbone_lora`` to immediately re-
    # freeze them — wasted work, misleading saved config.json, and a no-op
    # fp32 cast in ``_apply_backbone_lora`` since PEFT inherits fp32 from
    # the (already-promoted) base layer.
    ...
    # the vanilla Qwen3 path was removed in Issue #36. The CLI no longer
    # exposes --backbone-model-type; checkpoint values are ignored here and
    # any legacy "qwen3_vl" gets rewritten on disk by patch_checkpoint's
    # legacy_qwen3_vl_backbone handler at load time.
    ...
    # Issue #22 — plumb CLI fields that were previously silent no-ops because
    # assembly had no copy site.
...
# the release codebase fixes video on (Issue #36 / PR #39 dropped vanilla
# Qwen3, Issue #53 dropped the ``--use-video`` CLI knob), so video is now an
# architectural invariant — any checkpoint trained with ``use_video=False``
# is unsupported and rejected separately at the bottom of this function.
...
def _validate_features_match_ckpt(cli, loaded_yaml_config) -> None:
    ...
    # release-codebase VTC backbone. Pre-Issue#36 vanilla-Qwen3 checkpoints
    # are the only known case. Default ``use_video=True`` here matches the
    # post-Issue#53 ``RLDXConfig`` default, so missing-attr legacy configs
    # (e.g. very old yaml dumps without the field) are treated as VTC-shaped
...
def _apply_training_overrides(
    ...
    # Issue #22 — ``optim`` used to be unconditionally set to ``adamw_torch``
    # here, discarding both the fused default on ``TrainingConfig`` and any
    # value the user passed via ``--optim``. Copy the CLI value now that
    # ``optim`` is exposed on ``TrainConfig``.
