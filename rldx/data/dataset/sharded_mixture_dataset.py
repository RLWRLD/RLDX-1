...
from rldx.data.interfaces import BaseProcessor, ShardedDataset
        ...
        for emb, stats in all_stats_by_emb.items():
            ...
            for modality in all_modalities:
                if modality in stats[0]:
                    # modality_stats = [s[modality] for s in stats if modality in s]
                    # stats_by_emb[emb][modality] = merge_statistics(
                    #     per_dataset_stats=modality_stats,
                    #     dataset_sampling_weights=[
                    #         w for w, s in zip(weights_by_emb[emb], stats) if modality in s
                    #     ],
                    #     is_relative_stats=(modality == "relative_action"),
                    # )

                    # ── TY: Skip all-zero stats dataset ──────────────────────
                    valid_stats = []
                    valid_weights = []
                    for s, w, ds in zip(stats, weights_by_emb[emb], self.datasets):
                        if modality not in s:
                            continue
                        modality_stat = s[modality]
                        all_zero = all(
                            np.all(np.array(v) == 0)
                            for group_stats in modality_stat.values()
                            for v in group_stats.values()
                        )

                        rank_str = f"[Rank {self.rank}] " if hasattr(self, "rank") else ""
                        ds_path = getattr(ds, "dataset_path", "unknown")

                        if not all_zero:
                            print(
                                f"{rank_str}[merge_statistics] INCLUDE emb={emb}, modality={modality}, ds={ds_path}, weight={w:.4f}"
                            )
                            valid_stats.append(modality_stat)
                            valid_weights.append(w)
                        else:
                            print(
                                f"{rank_str}[merge_statistics] SKIP (all-zero) emb={emb}, modality={modality}, ds={ds_path}"
                            )
                    # ───────────────────────────────────────────────────────────────

                    if not valid_stats:
                        print(
                            f"[merge_statistics] No valid stats for emb={emb}, modality={modality}, skipping"
                        )
                        continue

                    stats_by_emb[emb][modality] = merge_statistics(
                        per_dataset_stats=valid_stats,
