...
import numpy as np
from rldx.utils.video_utils import get_accumulate_timestamp_idxs
...
class VideoRecordingWrapper(gym.Wrapper):
    def __init__(
        ...
        **kwargs,
        ...
        self.episode_id = kwargs.get("start_episode_id", -1)
        self.name_prefix = kwargs.get("name_prefix", "episode")
        # Optional: deterministic seeding per episode (useful when resuming / skipping episodes)
        # If set, reset() will set seed = base_seed * seed_stride + episode_id unless caller provides seed explicitly.
        self.base_seed = kwargs.get("base_seed", None)
        self.seed_stride = int(kwargs.get("seed_stride", 100000))
    ...
    def reset(self, **kwargs):
        # Deterministic reset seeding (only if caller didn't specify a seed)
        if kwargs.get("seed", None) is None and self.base_seed is not None:
            try:
                base_seed_int = int(self.base_seed)
                ep_id_int = int(self.episode_id)
                episode_seed = base_seed_int * self.seed_stride + ep_id_int
                kwargs["seed"] = episode_seed
                # Also seed process-global RNGs to make episode generation reproducible even if env uses np/random globals.
                random.seed(episode_seed)
                np.random.seed(episode_seed)
                # NOTE(SH): robosuite / robocasa environments maintain their own internal RNG (self.rng),
                # and reset() does not receive a seed argument (robosuite API), so for robust per-episode restarts,
                # we must also reset the internal RNG here.
                #
                # - PnP environments are less affected by randomness, so this is less noticeable,
                # - PosttrainPnPNovel environments sample layouts/styles/objects using self.rng more extensively,
                #   so issues can arise with restart/resume if the seed is not correctly managed.
                try:
                    # Descend through env wrappers (e.g., OrderEnforcing) created by gym.make() to find the actual robosuite env
                    unwrapped = getattr(self.env, "unwrapped", None)
                    robosuite_env = None
                    if unwrapped is not None and hasattr(unwrapped, "env"):
                        # robocasa gym wrapper: unwrapped.env is the robosuite env
                    ...
                    if robosuite_env is not None:
                        if hasattr(robosuite_env, "seed"):
                            robosuite_env.seed = episode_seed
                        if hasattr(robosuite_env, "rng"):
                            robosuite_env.rng = np.random.default_rng(episode_seed)
                except Exception:
                    # Seeding is best-effort; different envs may have different structures.
                    pass
            except (TypeError, ValueError):
                # If seed conversion fails, fall back to default behavior
                pass
        ...
        if self.video_dir is not None and self.file_path is not None and self.file_path.exists():
            ...
            new_filestem = f"{original_filestem}-"
            new_filestem += "success" if self.is_success else "failure"
        ...
        if self.video_dir is not None:
            self.episode_id += 1
            filename = f"{self.name_prefix}-episode_{self.episode_id}.mp4"
