...
"""
SO100 Real-Robot RLDX Policy Evaluation Script

This script runs closed-loop policy evaluation on the SO100 / SO101 robots
using the RLDX Policy API.

Major responsibilities:
    • Initialize robot hardware from a RobotConfig (LeRobot)
    • Convert robot observations into RLDX VLA inputs
    • Query the RLDX policy server (PolicyClient)
...
class So100Adapter:
    ...
    Adapter between:
        ...
        • RLDX VLA input format
        • RLDX action chunk → robot joint commands
    ...
    def obs_to_policy_inputs(self, obs: Dict[str, Any]) -> Dict:
        """
        Convert raw robot observation dict into the structured RLDX VLA input.
        """
        model_obs = {}
