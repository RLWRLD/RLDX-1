...
Beyond the LeRobot-format parquet/video files, every dataset RLDX
...
For memory-constrained finetunes you can replace full-parameter
training of either the action model (MSAT) or the backbone (Qwen3 LLM)
...
# Backbone LoRA — replaces top-N LLM layer tuning
--backbone-use-lora \
...
| `--backbone-use-lora` | False | Inject LoRA into Qwen3 attention + MLP projections. **Overrides `--tune-top-llm-layers`** — when on, the backbone is frozen and only LoRA adapters train. |
| `--backbone-lora-rank` | 16 | Adapter rank. |
| `--backbone-lora-alpha` | 32 | Scaling factor. |
| `--backbone-lora-num-layers` | -1 | `-1` = inject into all LLM layers; `N > 0` = inject into the top `N` layers only; `0` = skip backbone LoRA. |
...
[arXiv 2512.05964](https://arxiv.org/abs/2512.05964)) bakes
